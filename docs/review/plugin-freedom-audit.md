...
**不要再绑定邮箱**、注册时自填邮箱/用户名/密码、之后可在账号页修改。逐条确认后定为：
...
- **后端**：签发端点的 `email` 变**可选**（填了仍照常校验格式 —— "可选"不等于"不校验"，
  ...
  **新增 `POST /api/auth/profile`**（改邮箱/用户名），必须验当前密码、唯一性对齐
...
- `bash packages/plugin-org/test/e2e-p2-org.sh`：**73 项全过**（含通用码自选邮箱、
  不传邮箱报错、定向码不匹配 403、改资料全链路：错误密码 401 / 邮箱被占 409 /
  新邮箱能登录 / 旧邮箱不能）。
...
用户的原话：「要加邀请码管理，而不是显示一次就没了」。

原状确实如此：原始令牌只在签发那一次响应里出现，库里只有它的 sha256（与会话 cookie
同一纪律：DB 泄露不足以让人冒用），列表端点也刻意不回令牌 —— 于是管理员关掉那个提示框之后
**再也拿不到那个码**，只能"撤销 + 重建一条邀请"，而那样会丢掉这条邀请已经带上的
`orgRole` / `groupId` 配置，还在列表里平白多一行已撤销的垃圾。

**岔路与选择**。有两条路：把令牌**明文存库**（列表里直接显示）、或者**换发**（点一下生成新的）。
选了后者，理由是一条真实的安全性质：明文存库会让一份数据库泄露（备份、只读账号、注入读路径）
直接变成**可开号**的凭据，而换发让"随时能拿到一个可用的码"与"库里不存明文"同时成立。

- **后端**：新增 `POST /api/org/invitations/:id/rotate`（admin）。三条边界都写进了端点注释，
  且都是"写错了不会报错"的那类：
  1. **只能换发未接受且未过期的**。已接受的换发等于把一次性的邀请重新变成可用 ——
     会让**第二个人**凭同一条邀请进来，直接破坏"一人一码"；
  2. **已过期的拒绝，而不是顺手延长有效期** —— 延长是另一个决定（"再给他 7 天"），
     混进换发里会让"我只是想再复制一次链接"变成"我悄悄给他续了 7 天"；
  3. **换发 owner 邀请需要 owner**，与签发时同一判据 —— 否则 admin 只要找到一条 owner 邀请
     （哪怕是别人签的），换发一下就能自己用掉，绕开"admin 不得签发 owner 邀请"那条限制。
- **前端**：列表每一行多一个「获取邀请码」入口，**只对"待接受"的行渲染**（已接受/已过期的
  换发会被服务端 409 拒掉，不做一个点了必然报错的入口）；点击前确认一次，确认框里点明
  "会**立即作废上一个**"—— 否则管理员会把两个码都发出去，而旧码在对方那里是直接报「邀请无效」。

**验证**：`e2e-p2-org.sh` **85 项全过**，其中换发 9 项覆盖了四条边界（旧码作废、
新码可用、已接受 409 `already_accepted`、已过期 409 `invitation_expired`、member 403、不存在 404），
且审计里有 `org.invitation.rotate`、`leak=0`（令牌不入审计）。
`pnpm run test` **2238 例 / 2238 通过 / 0 失败**（新增 3 条守卫，含"列表数据里不得有令牌字段"）。
渲染实测（headless Chrome 打桩 fetch，走完"点按钮 → 确认 → 换发 → 显示新链接"全链路）：
待接受行有且仅有 1 个入口、已接受/已过期行没有、确认框有「重新生成」、发出了 rotate 请求、
显示了新令牌与 `#/invite/<新令牌>` 链接、提示语为"已重新生成……（上一个已作废）"。
...
- 改邮箱**不吊销其它会话**（与改密码对应的是不同威胁，理由写在端点注释里）。
