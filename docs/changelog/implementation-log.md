...
- **附件上传去掉大小上限：两道闸出厂改为「不限」（2026-09-21）**：用户要求「把附件上传的大小上限**也**去掉」
  （"也"指的是下面那条 dock 贴图记录里顺口提到的那句"wiki 附件只有一道 25 MB 上限"）。动手前问清了一件事：附件其实有
  **两道**大小闸——单文件 `attachmentMaxBytes`（25 MiB）与**单页总量** `attachmentPageQuotaBytes`（200 MiB），
  只去前者会让"同一页传到第 4~5 个大文件"照样被页配额挡下 ⇒ 用户选**两道都去掉**。
  - **口径是"闸门打开"，不是"闸门拆掉"**：`packages/plugin-wiki/src/attachments.ts` 的
    `DEFAULT_MAX_BYTES = 25 MiB` 换成 `NO_SIZE_LIMIT = 0`，两个配置项改成 `.default(NO_SIZE_LIMIT).min(0)`。
    **原先硬编码的 `.max(200 * 1024 * 1024)` 随之删除**——它的原意是"不让配置随手调到几个 GB"，
    但默认既是"不限"，再给配置值设上界就自相矛盾（想设一个比 200 MiB 更大的限额本来也是合法的运维决定）。
    ⇒ 运维把任一项设成**正数**即立刻恢复限额，不需要改代码发版。留着这条计数逻辑还连带留住了它旁边两道防线：
    `expectedBytes`（实收 vs 声明）对照与超限时**中断收流**（不是收完再判）+ 临时文件清理。
  - **`0` 的含义是"跳过这一道判据"，绝不是"拒收一切"** ⇒ 三处判据一律写成 `limit > 0 && …`：
    `attachment-store.ts` 的流式字节计数、上传端点的 `Content-Length` 前置闸、单页配额（前置检查 +
    写入事务里那次**权威**判定）。反向写（`size > limit`）会让**出厂默认**表现成"传什么都超限"，
    而配置里什么都没写 ⇒ 是这次改动里唯一能一击致命的写法。**三处统一成同一行内的守卫形态**，
    于是"漏掉守卫"退化成一条按行的源码守卫就能查出来的事
    （`test/attachments.test.ts` 的 `★ 出厂默认是「不限」…`；它当场逮到了我第一版把事务判定写成
    `if (quota > 0) { … }` 块状守卫的不一致——块状没错，但"每处自带守卫"才可被机器核对）。
  - **为什么"去掉"在结构上是安全的**：附件字节**从未经内存**——`storeStream` 逐块 `write` + 等 `drain`，
    所以旧注释里"并发上传打满内存"那条顾虑本来就不成立，真实代价只有磁盘。写满时端点回
    **503 `storage_unavailable`**（不是 500：5xx 会计入连续失败、可能把"磁盘满"升级成整站熔断），
    且**已有附件的读取不受影响**。不限大小之后，`Content-Length` 仍然必填，但**理由换了**：
    不再是"上限无法前置判定"，而是"无法与实收字节数对照"（`length_mismatch` ⇒ 400）——文案已跟着改。
  - **仍然拦得住大文件的四道闸**记进 `docs/design/attachments.md` §6.1 新增表：磁盘余量 /
    **反代的 body 上限**（nginx `client_max_body_size` **默认 1 MB**，超限的请求根本进不到 Node，
    响应也不是本服务那个带 `error` 字段的形状 ⇒ "上传莫名 413"的新经典成因；Caddy/Traefik 默认值**未实测**）/
    `Content-Length` 必填 / 扩展名白名单（415，且**不收一个字节**）。运维口径补在 `docs/deployment.md` §5
    （磁盘要自己盯 + 一段"想收口就 POST 插件配置"的命令）。
  - **代价明说**：**并发上传闸门**（U14）、**上传超时**（U8）、**全库配额**（U5）这三项本就未实现，
    不限大小之后代价变高——今天一个**已登录**用户可以不间断地往磁盘里写。匿名流量仍在网关层 401，
    所以"不限"目前只对已鉴权流量成立。
  - **读数**：附件端到端 `bash packages/plugin-wiki/test/e2e-attachments.sh` **177 通过 / 0 失败 / 1 跳过**，
    其中新增的**阶段 U** 用真 HTTP 传了 **26 MiB 单文件**（越过旧默认 25 MiB）与**同页累计 216 MiB**
    （越过旧页配额 200 MiB）⇒ 都 201，下载回来的字节数与源文件逐字一致、`tmp/` 零残渣；
    **同一份 26 MiB 文件**在把 `attachmentMaxBytes` 配回 1 MiB 后立刻 **413 `payload_too_large`**
    ——这一条是整套验收的重点：它证明改的是"闸门开合"，不是把闸门拆了。
    `@geewiki/wiki` 单测 **144/144**；全仓 `pnpm test` exit 0、`pnpm typecheck` 全绿、
    `pnpm lint`（`--max-warnings 0`）零告警。
  - **有意更新的既有守卫**：`test/attachments.test.ts` 里 `assert.equal(DEFAULT_MAX_BYTES, 25 * 1024 * 1024)`
    随常量改名变成 `assert.equal(NO_SIZE_LIMIT, 0)`（保留这一行，让"默认值变了"在测试里可见而不是悄悄消失）；
    端到端脚本头部对"超限 413 / 配额 413"两条断言的说明补了"靠脚本自带的窄配置触发"。
    **未做**：`repoQuotaBytes`（全库配额）、并发闸门、上传超时——见上；反代上限的实测（本机没挂 nginx）。

- **dock 贴图不再压缩：原图直传（2026-09-21）**：用户要求「我希望照片上传后不会被压缩」。
  动手前用一次问答定档两件事：**范围**＝AI 助手 dock 的贴图（wiki 附件那条链路**本来就不压**——
  `packages/plugin-wiki/src/attachments.ts` 只有一道 25 MB 上限、全程无 canvas，`packages/web/src` 里
  也搜不到重编码 ⇒ 那条路**一字未动**）；  〔2026-09-21 同日补充：那条链路随后被要求**去掉大小上限**，见本页更上面那条〕；**策略**＝**完全不压、原图上传**（在看清代价之后选的）。
  - **上传路径只剩一次读取**（`packages/plugin-ai-assistant/ui/index.tsx` 的 `prepareImageFile`）：
    原先"长边 >1280 或字节超 400 KB ⇒ canvas 重编码成 JPEG q0.82"整段撤掉，现在是 MIME 校验 →
    `readAsDataUrl` → 按 `IMAGE_MAX_BASE64_CHARS` 判一次 → 超了**当场拒绝**（文案点出是哪一张、有多大、
    上限多少，并写明"本站不会替你压缩照片"）。判据落在**真正发出去的那份载荷**（base64 字符数）而不是
    `file.size`：按 size 放行、按载荷拒绝会造出"挂上了却发不出去"的窗口。唯一的画布重编码留在
    `makeImagePreview`，那份**只进 localStorage、永不进请求体**。
  - **不压图 ⇒ 落盘必须另存一份副本**（`ui/imagePlan.ts` 新增 `DockImage.preview` 与 `imagesForStorage`，
    `ui/dockPlan.ts` 新增 `withStoredImages`）：原图 2~8 MB 很常见，而 localStorage 通常只有 5 MB——
    直接写原图的结局不是"存下来了"，是**配额降级把整段历史的图全丢掉**。故 `saveConversation` 落盘前把
    `url` 换成副本，**存储形态仍是 `{url, name}`**（读侧 `normalizeDockImages` 与所有渲染都不认识"副本"，
    旧数据照读），`preview` 字段本身**绝不落盘**（否则原图与副本各占一份配额），`withoutImages` 退成**第二道**降级。
    代价写明：**刷新之后**那一格里是副本，于是刷新后的 `image.save` 存的也是副本——这与改动前逐字相同，
    不是新增的退化；要原图就在这一轮里当场存。
  - **一组数字必须同时抬**（`src/types.ts`、`src/index.ts`、`ui/imagePlan.ts`）：单图 1.4M → **11M 字符**
    （≈8.25 MB，按"手机相机原图直传"取值）、`MAX_BODY_BYTES` 16 MB → **48 MB**、`MAX_CONVERSATION_IMAGES`
    **8 → 4**。第三条是前两条的**必然结果**而非妥协：4 张原图 ≈ 当年 8 张压缩图的字节量，
    `4 × 11M = 44M ≤ 48M` 成立，请求体的量级没变，代价换成"一轮最多 4 张"。**没有选"抬总闸保住 8 张"**——
    body 是整体缓冲后 `JSON.parse`，`8 × 11M = 88M` 在 `MAX_CONCURRENT_STREAMS`(4) 路并发下是几百 MB 驻留。
  - **刻意没跟着涨的**：`AI_TOOL_IMAGE_MAX_BYTES` 仍是 1 MB（那是模型自己截的屏，不是用户的照片）；
    `MAX_TURN_TOOL_IMAGES` 仍是 8，顺手把它注释里"与 `MAX_CONVERSATION_IMAGES` 同值不是巧合"那句
    **已经失效的镜像说法删掉了**（注释撒谎比没有注释糟）。
  - **两个既有守卫被有意更新**（不是回归，改的是守卫本身）：`⑤` 里
    `humanBytes(IMAGE_KEEP_BYTES) === '400.0 KB'` 随常量改名变成 `IMAGE_PREVIEW_KEEP_BYTES` / `'512.0 KB'`；
    `test/loop.test.ts` 那张"超长"的图从写死的 `'A'.repeat(1_400_001)` 改成 `MAX_IMAGE_BASE64_CHARS + 1`
    ——写死数字的话，闸一抬高这条守卫就**悄悄失效**（它正是这次跑测时先炸出来的）。
  - **新增守卫**：`⑥ ★ 上送的那一份必须是原图`（整份 UI 只准有一处 canvas 重编码、且必须在
    `makeImagePreview` 内，上传函数体内不得出现；外加拒绝文案三要素：哪一张 / 上限多少 / 本站不压图）；
    `④ ★★ 落盘走副本、上送走原图`（两个方向各一条断言，因为两边都**不报错**）；`②` 新增
    "单图解出来不足 8 MB 就是假承诺"。这批守卫共同针对的失败面是"图看着在、模型收到的是糊图"。
  - **读数**：`pnpm typecheck` 全绿、`pnpm lint`（`--max-warnings 0`）零告警、`pnpm test` 全仓绿（exit 0）；
    `@geewiki/ai-assistant` **272/272**。设计说明同步在 `docs/design/dock-images.md`
    （§0 第三批口径与原话、§2 第 2/3 跳、§4 常量表与两条不等式、§6 副本与两级降级、§9 新增 L11–L13）。
  - **未做 / 未实测**：浏览器端到端（真机贴一张 4 MB 原图走一遍）与真实上游视觉调用都没跑。
    上游网关对单请求体 / 单图另有上限、**具体值未知**，"本地放行而上游 400"是这次改动直接引入的风险面（§9 L12）；
    EXIF 方向此前被那次重编码顺手纠正，现在没有这一步了（§9 L13 ③，也未实测）。
...
- **输入条支持图片（2026-09-20）**：用户原话「当前在dock栏还无法输入图片给ai，加一下此功能」。
  ...
  **模型要有把图存进服务器的能力**（用户原话口径："比如说用户要求把图片插入进文章的时候"）。
  ...
  - **七跳链路**：三个入口（文件选择 / 粘贴 / 拖拽）→ `prepareImageFile`（超 400 KB 才重编码到
    JPEG，**GIF 永不重编码**）〔**2026-09-21 这一次重编码已按用户要求撤掉**——现在原图直传、
    重编码只留一份本地副本，见本页顶部「dock 贴图不再压缩」〕→ `DockImage{url}` → `trimConversationImages` → `{mime,data}` →
    服务端 `parseImages`（白名单 + base64 形状 + 长度，**不解码**）→ `toLlmMessages` 折 data URL。
  - **一组必须同时成立的数字**：单图 1.4M 字符 / 单条消息 4 张 / **整段对话 8 张** / 请求体 16 MB。
    〔这组数字 **2026-09-21 已整体改为 11M / 4 / 4 / 48 MB**，理由与自洽等式见本页顶部〕
