...
> 它在 README 里从 2026-09 起逐批追加，最终长到 **834 行 / 约 220 KB**，把「快速开始」挤到了第 841 行。
> 2026 年整改时**逐字迁出**到这里（源：`README.md` 第 7–840 行），内容未删改。
>
> **为什么迁出**：README 的职责是回答「现在是什么、怎么跑起来」。变更流水越长，这两件事越难找；
> 而流水本身有保留价值（它记着**为什么**这么改、以及当时的验证读数），所以是**搬家**不是删除。
>
> **注意口径**：本页是**历史快照**，其中的读数、插件数量、行号在被写下的那一刻成立，之后可能已过时。
> 需要当前读数请看 [test-ledger.md](test-ledger.md) 与 README 的「质量基线」，或直接跑 `pnpm test` / `pnpm run typecheck`。

---

**当前状态：Phase 2 完成 + Phase 3 主体落地** —— ⚠️ **本条是 AI 插件化重构之前的口径**（下文凡提 `@geewiki/ai-qa` / `wiki-ask` / `#/wiki/ask` / `POST /api/ai/stream` 的段落均已过时），**以紧邻下方的「AI 插件化重构（P8 已完成）」块为准** —— 插件管理器（依赖图/冲突组/会话层/看门狗）、Wiki CRUD + 版本历史、React 19 管理台与依赖图均可完整交互；插件配置系统（schema 表单 + 热更新）、外部插件目录发现、前端 Slot（**宿主侧插槽 + 后端下发入口表 + 随插件生命周期自动同步**）与**后端插槽注册链路**（`ctx.get('slot')` / `SlotService`：`contribute` / `list` / `ownersOf` / `release`，含单占用基数裁决与 owner 级回收）均已落地；**全文检索已默认启用**（SQLite FTS5 + `trigram` 分词器解决中文检索 + 短查询 LIKE 兜底），**检索增强问答（RAG）已重写为"模型问答"形态**（本批把原 `@geewiki/ai` 一分为二：`@geewiki/ai-qa` 做问答、`@geewiki/ai-writing` 做辅助写作；**"没有 API key 也完整可用"这条承诺在问答侧已撤销**——旧做法是没有模型时把抽取式检索摘要塞进同一个 `answer` 字段，用户读到的仍是"像答案"的文本，详见下方 AI 两条）**；**LLM 厂商 adapter 已落地**——OpenAI 兼容 adapter `@geewiki/openai`（`packages/plugin-openai/`）已提交并登记进 `defaultRegistry()`，向 `llm-service` 注册一条真实路由；**本批起模型接入的配置全部收进 `@geewiki/llm` 一处**（服务商下拉 + 端点 + **写一次不可回读的密钥** + 模型名 + 上下文长度 + 最长输出 + 思考强度），适配器**不再有自己的配置项**，**AI 四件套 `@geewiki/llm` / `@geewiki/openai` / `@geewiki/ai-writing` / `@geewiki/ai-qa` 默认启用**（没有密钥时问答与辅助写作**明确报不可用**，不再假装能用；检索不受影响），见下方「如何接入真实模型」；**SSE 流式问答已交付**：`POST /api/ai/stream` 逐帧下发（事件名共 4 个：`status` / `delta` / `done` / `error`，见 `packages/plugin-ai-qa/src/sse.ts:49-52`），**前端按帧增量渲染现在也归插件**（`packages/plugin-ai-qa/ui/sse.ts` + `ui/index.tsx`，经 `wiki-ask` 插槽插入宿主；宿主内已无问答界面代码）。开发路线与阶段进度见 [docs/roadmap.md](docs/roadmap.md)（**注**：该文档与 [docs/plugin-platform-plan.md](docs/plugin-platform-plan.md) 的若干段落仍停留在更早批次的口径——把厂商 adapter、`ctx.slot()`、真实流式输出等记为"未做"；**以本 README 的实测结论为准**）。
...
- **AI 回退之后正文自动刷新（2026-09-16）**：用户原话「现在是回退后不会自动刷新」。
  ...
  - **⚠️ 我这次又踩了那个已经记在案上的坑**：只跑 `pnpm --filter @geewiki/web build:plugin-ui`
...
- **AI 助手的空闲计时器从未被重置 + 「继续」按钮根本不存在（2026-09-16）**：用户看了上一批的截图，两条反馈都对：
  ...
  - **验收**（`data/verify/ai-refresh/long-turn.mjs`）：同一个"读 8 篇再总结"的请求，改之前**30.002 秒**被
    ...
    ⚠️ 这台实例的模型网关当前确实不稳（`PROVIDER_ERROR`／早前见过上游 500），所以"整轮成功"这一条
...
- **AI 助手：卡片表面修回外壳内（2026-09-16，用户报"对话框是透明的"）**：用户截图反馈「这对话框现在样式不对啊，
  ...
  - **读数**：`pnpm test` **1858/1858**（24 个测试包，0 fail；`packages/plugin-ai-assistant` **220**）、
    `pnpm -r typecheck` **27 Done / 0 error TS**。
- **AI 助手：收起态与展开态的输入框**合并成同一个盒子**（2026-09-16）**：用户原话「感觉整体动画的衔接还是有问题，
...
- **插件依赖图大改：管理面合并 + 可读性 + 真·临时停用（2026-09-15）**：用户提了四件事——
  ...
  - **★ 真·临时停用（后端能力，不是只上色）**：此前 `disable()` 对基础层插件直接 409 `base_layer`，
    ...
    **11/11** 真进程验收（含"两个清单文件逐字节未变"与"真重启后恢复"）。
    ⚠️ 该验收脚本第一版**假绿**：`pnpm` 只是包装，`kill(pnpm)` 杀不掉真正监听端口的孙进程 ⇒ "重启"成了"同一进程继续服务"
  ...
  - **★ 用户实测返工二：临时停用不进「临时变更」，且停用后**启不回来****（同批）：
...
- **AI 插件化重构（P8 已完成 —— 拆除旧问答 UI + 删除 `@geewiki/ai-qa`）**：
  ...
  **④ 两处被既有测试当场抓出的连带缺陷（不是事后自查发现的）**：删掉解析分支后
  ...
  一个检索服务，而是**必需的那几条工具**）。
  **⚠️ 上文凡提到「`@geewiki/ai-qa` / `wiki-ask` 插槽 / `#/wiki/ask/<q>` 路由 /
  `POST /api/ai/ask` / `POST /api/ai/stream` / `.ask-*` 类名」的段落均已过时**，
  它们记录的是本批之前的口径，保留以存史。设计真源见
  [docs/design/ai-plugin-architecture.md](docs/design/ai-plugin-architecture.md)
...
- **点 dock 外部即收起（2026-09-15，用户要求「当点击 dock 外的时候直接收起，而不是一定要点收起按钮」）**：
  ...
  **顺带补 Esc**（同一件事的键盘对应物）：没有它，键盘用户只能 Tab 到"收起"图标才能关面板。
  ⚠️ **收起后不要把焦点还给输入条**：两个输入框的 `onFocus` 就是"展开"，还焦点等于 Esc 无效
...
- **AI 插件化重构（P5 已完成）**：本批落三件事，**前两件不需要模型密钥**。
  ...
  **⚠️ 上文所有更早的"15 / 17 个内置插件""启用 11 / 13 条""1478/1590 测试"读数均已被本段取代**，
  它们记录的是各批次当时的口径，保留以存史。设计真源见
  [docs/design/ai-plugin-architecture.md](docs/design/ai-plugin-architecture.md) §8.10。
- **AI 插件化重构（P6 已完成）**：需求 ③④ 落地——**每篇文章自动生成摘要、按摘要检索、
  ...
  **⚠️ 上文所有更早的"20 个内置插件""启用 18 条""1774 测试"读数均已被本段取代。**
  设计真源见 [docs/design/ai-plugin-architecture.md](docs/design/ai-plugin-architecture.md) §8.11。
- **AI 插件化重构（P4 已完成）**：新增**两个内置插件**，都进默认基础层清单（启用 14 → **16** 条）：
  ...
  - **`@geewiki/ai-pages`**（`packages/plugin-ai-pages/`，纯贡献者）——本仓**第一条 mutating 工具** `page.update`。
    ...
    ⚠️ **`wiki-service.save()` 不带主体**（授权在 HTTP 处理器里），所以工具必须自己向 `policy-service`
...
- **⚠️ 仍未做**：`page.update` 之外的页面管理工具（建页 / 删页 / 改可见性，归 P5 的 `ai-admin`）。
