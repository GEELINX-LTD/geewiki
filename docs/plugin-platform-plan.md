...
| 修改（**修正**，已落地） | `packages/manager/src/index.ts` —— `updateConfig` + 两个 REST 端点（注册在 `:1126` / `:1135`）；`snapshotOf` 携带 schema（**下发前剥离 `callback` / `preserve`，见 S-8**）。**原子写落在既有 `writeList()` 内**（`packages/manager/src/index.ts` 的 `writeList()` 已内联 `.tmp` + `renameSync`），**未新建 `packages/manager/src/config-store.ts`** |
