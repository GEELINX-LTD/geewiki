...
```
React 19 前端（宿主侧插槽：`app-header` / `app-footer` / `editor` / `editor-toolbar` /
        `app-dock` / `article-summary` / `account-identities` 七个内置插槽，另有插件自定义扩展点，
        详见第 6 章；「插件管理」与「依赖图」已合并为同一页 `#/plugins`）
        │
        │  REST API（HTTP）
        ▼
...
不进任何冲突组、可自由组合的服务提供方：
        ...
        AI 工具与会话层：@geewiki/ai-tools / ai-kb / ai-journal / ai-pages / ai-web-search /
...
3. **插件生态**：全部业务能力以插件形式存在。**按冲突组划分**的是真正互斥的同类实现——当前只有
   数据库组（`@geewiki/db-sqlite` / `@geewiki/postgres`，同属 `database-provider`，**冷切换**）；
   编辑器组（Milkdown / TipTap）**从未实现**，内置编辑器仍是宿主自带的 textarea（见 §9.7）。
   而**检索（`@geewiki/search`）、LLM 契约层（`@geewiki/llm`）与各 AI 插件刻意不进任何冲突组**——
   它们提供的是可被多方复用的服务。`@geewiki/openai` 同样不声明冲突组：多个适配器是「模型接入」
   `provider` 字段**单选**的并列服务商，同时启用是正常需求（多家共存、各自注册一条路由），
   而不是冲突。
...
- 生产环境切换至 **@geewiki/postgres**（目录 `packages/db-postgres/`；**不是** `@geewiki/db-pg`）时，通过 Docker Compose 的 `production` profile 启动 postgres 服务（见第 8 章与根目录 docker-compose.yml）；两个数据库插件都声明 `runtime.supportsHotReload: false`，**切换是冷操作**。**与 `@geewiki/search` 不兼容**（FTS5 是 SQLite 专有）。
...
因此**必须**另开一条"只记指标、绝不碰响应"的通路，即 `RouteHandlerContext.noteStatus?(status)`（`packages/core/src/index.ts:976`）。它与 `json()` **共用同一记账点**（`packages/server/src/index.ts:272-292`）——`settledStats` 幂等守卫保证**每个请求只记一次**：长连接先 `noteStatus(200)` 记状态码、随后（如异常收尾路径）仍可能经 `json()` 再走一次，重复记账会让 `stats()` 总数虚高、看门狗连续失败计数被放大。

长连接出口的正确姿势：**确定状态码时调用 `noteStatus()` 记一次指标 → 自行持续写帧 → 自行 `res.end()` 收尾。**

**（c）`trackStream` / `closeStreams`**

- `HttpRouterService.trackStream?(res, owner?)`（`packages/core/src/index.ts:1172`）登记长连接，返回**幂等**注销函数（`packages/server/src/index.ts:170-178` 用 `released` 布尔守卫）。登记的**唯一价值**是让路由服务在**自身卸载/关停**时能主动结束这些连接——否则客户端会一直挂着等一个再也不会来的字节。持有者若要按自己的生命周期收流，调用返回的注销函数即可。传 `owner`（通常是插件名）的连接，由路由服务在**卸载该 owner** 时定向回收。
...
- `closeStreams?(owner?)`（`packages/core/src/index.ts:1184`）与 `trackStream` 一样**是接口上的可选成员**——其他包经 `ctx.get('http')`（静态类型 `HttpRouterService`）拿到的服务上就有这个方法。不传 `owner` 关全部，传 `owner` 只关该 owner 开的那些。实现为逐个 `res.end()` 并 `try/catch`（`console.warn('[@geewiki/http] 结束长连接失败:', err)`），单个连接异常不得阻断其它连接的收尾；最后 `clear()`。
- **接口上另有两个配套成员**：`noteStreamRejected?()`（`packages/core/src/index.ts:1195`，只上报"因并发上限被拒"）与只读统计类型 `HttpStreamStats`（`:1065`）。四个 `?` 可选成员都保持向后兼容——既有测试替身无需改动。
...
- **加载时**：自动递归加载所有未激活的依赖项。
- **卸载时**：计算下游依赖者（反向依赖）；若存在依赖者，则阻止卸载并给出**专门说明块**（`409 has_dependents`）。管理台不再只显示泛化错误：`packages/web/src/pages/AdminPage.tsx:423-455` 的 `.dependents-block` 列出依赖方名单（来自响应 `details.dependents`；形状不符时退化为"后端未返回名单"的兜底文案），并给两段**可操作指引**——① 先在上表逐个停用依赖方，再回来停用目标；② 若目标是被同冲突组的其它插件顶替，可在目标插件那行点「启用」走**冲突组替换**（会连同依赖方一起安全接管）。**刻意不实现自动级联停用**（破坏性操作，刻意不做）。
...
- **真·临时停用**：此前 `disable()` 对**基础层**插件直接 409 `base_layer`（"冷操作，请改清单后重启"），
...
「插件管理」与「依赖图」两个页面已合并为一页（`packages/web/src/pages/GraphPage.tsx`）：**图即管理台**，
...
- **线不再交织**：分层只解决"被依赖方在左"，**层内顺序**才是交叉的来源（原先沿用注册表顺序）。
  ...
  （横向区间不重叠的边对不计——用"全部对"当目标会去优化肉眼看不见的交叉，实现时先踩过这个坑）。
...
> 历史引用说明：本文档（及 `docs/plugin-platform.md`）里凡引用 `packages/web/src/pages/AdminPage.tsx`
> 的**行号与结构**，都是**合并前**的记录；该文件已 `git mv` 为 `pages/GraphPage.tsx`（画布抽到
> `components/PluginGraph.tsx`），行号不再对应。**主路由是 `#/plugins`**；老链接 `#/graph` 在路由解析处
> 改写成 `#/plugins`（`packages/web/src/App.tsx:316`：`const root = root0 === 'graph' ? 'plugins' : root0`），
> 书签仍可用。两个 id 都在 `RESERVED_ROUTE_IDS`（`packages/core/src/domain.ts:500-504`）。
...
> **注意两个端点的 `layer` 不同义**：配置端点的 `layer` 是持久化层，列表端点 `GET /api/plugins` 的 `PluginSnapshot.layer` 仍是激活层（未激活 `null`）。判据见 `docs/plugin-platform.md` 批次 C 的端点契约。
...
> **插槽一节的口径（以源码为准）**：内置插槽白名单共 **7 个**，真源是
> `packages/core/src/slots.ts`（`BuiltinSlotName` 在 `:69`、`SLOT_NAMES` 在 `:94`、
> `SLOT_CARDINALITY` 在 `:138`）——**不是** `packages/core/src/index.ts`（它只 `export *` 转出）。
> 七个槽与基数：`app-header`(multi) / `app-footer`(multi) / `editor`(single) /
> `editor-toolbar`(multi) / `app-dock`(single) / `article-summary`(single) /
...
- **订阅式读取路径**：`pluginUi.ts` 原有的 `parseUiTable` 会**直接丢弃** `skipped`（没有对外可读入口），故补了最小订阅式读取（`pluginUiState()` / `subscribePluginUiState()`，`packages/web/src/lib/pluginUi.ts:148/159`；快照引用稳定、仅在变更后通知），管理台用 `useSyncExternalStore` 消费（`AdminPage.tsx:96`）——**没有新写第二份 fetch**。解析侧 `readSkipped` **刻意从宽**：`skipped` 坏掉绝不能让"加载/卸载"也判为不可信（否则一个展示字段会拖垮入口表关键路径）。
...
> 落在 Shadow Root 内的挂载点会被忽略，见 `isUsableMountTarget`）。
> **越权闸门（P12）**：入口表新增 `extNodes`（生效的非插槽节点），前端据此 + `suppressed` 拦下
> "被抑制 / 未获生效"的注册——此前闸门只认插槽裁决，非插槽节点上的被抑制 `replace` 仍会注册
> （症状是"后端说 A 生效、界面渲染了 B"）；受限宿主 `PluginUiHost` 同时补上了 `registerExtension`。
> 契约真源 [design/ui-extension-platform.md](design/ui-extension-platform.md)，验收读数见其 §11
> （`scripts/acceptance/ui-extension-cdp.mjs`，Chrome 151 headless，**28/28 全绿**）。
> 下面第 1、4 条已按此更新；第 3、5、6、7、8 条**仍然成立**。

1. **后端 `ctx.slot` 链路已建立，前端镜像与消费者已补齐；`admin-page-slots` 仍是候选**：后端已有 `SlotService`（`ctx.get('slot')` 可取，提供 `contribute(owner, slot, meta?)` / `extend(owner, node, mode, meta?)` / `list(slot?)` / `ownersOf(slot)` / `release(owner)`，且**插件卸载时按 owner 自动回收**；内置槽另可 `define` 出插件自定义扩展点），插槽基数由 `SLOT_CARDINALITY` 声明（真源 `packages/core/src/slots.ts:138`：`app-header` / `app-footer` / `editor-toolbar` / `account-identities` 为 `multi`，`editor` / `app-dock` / `article-summary` 为 `single`），`GeeWikiMeta.slots?: SlotName[]` 与新增的 `geewiki.extensions?: ExtDeclaration[]` 让插件可在 manifest 里声明占用，`GET /api/plugins/slots` 与入口表的 `slots` 字段对外可见（**`slots` 计入 `revision`**——否则"编辑器换人"会被 304 静默隐藏），诊断端点另增 `extensions` 字段（additive）。`app-header` / `app-footer` 的**零属性**语义**未变**；`editor` 是具名数据插槽（`EditorSlotProps`：`value` / `mode` / `slug` / `readOnly?` / `onChange` / `onSave` / `onCancel` 及一组可选通道），**刻意不留 `[k: string]: unknown` 逃生口**——加字段必须显式改类型。**仍未提供的是 `admin-page-slots` 这一个候选扩展点**；**外壳节点已全部接线**（7 个 `shell-*`：品牌两档 + `shell-header` / `shell-footer` / `shell-theme-toggle` / `shell-command-palette` / `shell-status-dialog`，守卫 `packages/web/test/shellChromeExt.test.ts`）。其中 `shell-header` / `shell-footer` 是**容器节点**（目录字段 `nestedSlots`）：`<header>` / `<footer class="app-footer">` 与 `app-header` / `app-footer` 插槽出口**由宿主独占**，`replace` 只换**内容**，贡献者用 `props.slots[插槽名]` 把出口摆进自己的标记里——**单个插件不可能删掉其他插件的贡献**（初版接线把整个 `<header>` 交给 `replace`，被用户当场驳回；机制见设计文档 §4.5）。原候选 `shell-sidebar` **经核实外壳里没有这个元素**（桌面导航是 `<header>` 里的横向 `<nav>`、阅读页右栏由 `wiki-toc` 覆盖），已从候选**移除**——没接线就不进目录，登记一个没接线的名字等于给插件一个静默失效的旋钮。**`PENDING_WEB_SYNC` 那套记账已随前端补齐而清空**（守卫现在要求它保持为空），消费 `editor` 的插件也已存在（内置 `@geewiki/editor-plain`）。同基数插槽（`single`）同时被多个 active 插件占用时，裁决规则是**激活顺序最早者胜出**、其余进 `slotConflicts`——选"最早"而非"最新"是因为最新胜出会让后启用的插件**静默顶掉**用户正在用的编辑器；但正解仍是作者用 **`conflictGroup`** 声明互斥（插件级互斥会在**启用时**就明确拒绝，而不是留到运行期裁决）。**`replace` / `wrap` 两种新模式的单占用裁决同此规则**（`resolveExtensions()` 是唯一裁决器，`suppressedDetail` 点名被谁顶掉）。
2. **没有 Suspense + use Hook 懒加载**：远端 bundle 由加载器显式 `import()`；"不白屏"靠上面的 ErrorBoundary，而不是 Suspense Fallback。**但"按需"是真的**：`ON_DEMAND_SLOTS = ['editor', 'editor-toolbar', 'app-dock', 'article-summary', 'account-identities']`（**5 项**，`packages/web/src/lib/pluginUiPlan.ts:172`）里的插槽，其 `client.js` 推迟到对应视图真正挂载时经 `ensureSlotLoaded(slot)` 拉起（`packages/web/src/pages/WikiPage.tsx` 的问答 dock 与编辑视图——编辑视图必须**成对**拉起 `editor` 与 `editor-toolbar`）。实现过程中为此付出过两个隐蔽的假缺陷，两条都值得记档：**① 入口可见性只能按入口表的声明 + 仲裁判，不能按"已注册组件"判**（`pluginUiDeclaredFor`，`packages/web/src/lib/pluginUi.ts:190`；`slots` 字段是后端裁决后的**生效集**，被抑制的单占用声明者不在其中）。按已注册组件判在懒加载插槽下会**自锁**：按钮只有"已经出现过"才会出现 ⇒ 列表页的入口恒不渲染，而管理台里插件是 active、入口表里条目也齐。**② 漏掉一句 `ensureSlotLoaded('editor-toolbar')` 的后果是一片静默空白**：入口表条目"生效"、无 console error、无网络请求，与"该插件根本没提供界面"长得一模一样。
...
4. **样式默认无隔离；`replace` 可选 Shadow DOM**：插件 CSS 仍以 `<link data-plugin-ui=…>` 全局注入文档，默认**不做任何样式隔离**（无前缀改写）。示例夹具以 `.gw-fixture-*` 前缀命名类名（`packages/web/fixtures/src/style.css`）作为**约定示范**。**P9 起** `registerExtension(node, c, { mode: 'replace', shadow: true })` 可把该贡献渲染进 Shadow Root：`--gw-*` 令牌跨边界继承（主题照常生效），宿主 Tailwind 类名**进不去**（CDP 实证：shadow 内 `.font-bold` 字重 400、宿主内 700），壳是 `<span data-ext-shadow style="display:contents">`（不生成盒子）。**仅 `replace` 有意义**（`wrap` 的宿主默认元素在 shadow 之外、`extend` 只是追加），其余模式传该标志会丢弃并告警。**另外**：插件 CSS 现在**禁止硬编码颜色**（`packages/web/src/lib/pluginCssPlan.ts` 静态扫描，守卫 `packages/web/test/pluginCssGuard.test.ts` 扫源码与 `plugins/*/dist/client.css`），因为写死的颜色会同时破坏深色模式、主题覆盖与对比度告警（后者看不见算不出来的颜色）。
...
6. **无版本协商与完整性校验**：插件 UI 目前没有签名与版本校验（`packages/web/src/lib/pluginUi.ts:41` 明确登记为未做的边界）。宿主 SDK 有 `HOST_SDK_VERSION`（现 **0.12.0**）与 `propsVersion`，但**没有强制的版本协商**——插件应特性探测而不是比版本号。**另一件容易误解的事（P13）**：加载期间的 `window.__GEEWIKI_HOST__` 是**按插件作用域**的宿主（与 `register(host)` 的参数是同一个对象），来源 = 插件名、过越权闸门、注销函数进同一份 `disposers`；`'host-sdk'` 这个来源只剩"没有插件作用域时"（宿主代码，或插件在加载结束之后才注册）这一种含义。
7. **import map 的浏览器基线**：import map 需要 Chrome / Edge 89+、Safari 16.4+、Firefox 108+（本项目未提供降级路径）。**本仓的浏览器侧验收（`scripts/acceptance/plugin-ui-cdp.mjs`、`scripts/acceptance/ui-extension-cdp.mjs`）只在 Chromium 上实测过**。
8. **验收脚本不接入 `pnpm test`**：`scripts/acceptance/plugin-ui-cdp.mjs` 零依赖（Node 内置 WebSocket + 直连 CDP），但需要 Chrome、一个运行中的实例与已构建的插件产物，属集成/验收层，故**刻意不进 `pnpm test`**（后者只跑无浏览器、无网络的单测）。**同款的还有** `scripts/acceptance/theme-header-cdp.mjs`、`editor-modes-cdp.mjs` 与 **`ui-extension-cdp.mjs`**（P4–P12 的端到端验收：replace/wrap/extend、shadow 隔离、主题令牌穿透工具类、外壳与容器节点、入口表与静态层、**插件 bundle 经受限宿主带 mode 注册**，**36/36 全绿**）。**启停插件需要 admin 会话**：由共用夹具 `scripts/acceptance/lib/session.mjs` 自动选择三条路径（启动实例时设 `GEEWIKI_ADMIN_TOKEN` 走 break-glass / 无账号时 `POST /api/auth/setup` / 固定验收账号登录），并自动带 `cookie` 与 `x-gw-csrf`；拿不到凭据时依赖鉴权的用例**明确跳过**并写进结果 JSON（不记失败、也不记通过）。**原先登记的两条缺口已关闭**（2026-09-21）：`plugin-ui-cdp.mjs` 已按上述夹具修复并跑通（陈旧的"wiki 有界面"断言换成"入口表列 `skipped(inactive)` + wiki 不再出现在 `loaded()`"）；插件 bundle → `registerExtension(mode)` 这条链已有真浏览器覆盖（`ui-extension-cdp.mjs` 的 P12b 场景 8 条）。**仍存在的缺口**是"顶层直接调全局 SDK 的注册没有归属"（见 design 文档 §9.11 与 roadmap 第 0 项第 5 件）。
...
**AI 导航与管理工具（P5 落地）**：内置插件 **`@geewiki/ai-nav`**
...
的键数不变。详见 [design/ai-plugin-architecture.md](./design/ai-plugin-architecture.md) §8.10。
...
> | 读数 | 值 | 明细 |
> | --- | --- | --- |
> | 内置插件注册 | **25** | `defaultRegistry()` 静态登记；另有 `plugins/` 下 2 个外部示例（`hello-geewiki` / `ui-demo`） |
> | 默认启用 | **21** | `db-sqlite http auth org authz ops wiki builtin-docs search` + `llm openai ai-tools ai-journal ai-kb ai-web-search ai-summary ai-pages ai-assistant ai-writing ai-nav ai-admin` |
> | 已注册未启用 | **4** | `echo editor-plain oidc postgres` |
>
> 零依赖示例见 `plugins/hello-geewiki/` 与 `plugins/ui-demo/`。
>
...
> 上例的 `configSchema` 是 schemastery 实例（JS 对象），JSON 清单无法表达，故以字符串示意写法；实际写法见 `packages/plugin-echo/src/index.ts` 的 `EchoConfigSchema`（当前内置插件仍以 JSON Schema 风格字面量承载，契约迁移见 `docs/plugin-platform.md` 批次 C 的"现状与契约缺口"）。外部插件的清单若用 JSON 描述，可省略 `configSchema`（该插件将只获得 JSON 原文配置编辑，不做校验与裁剪，见 5.7）。
...
> `@geewiki/ai-web-search`（`web_search` 联网检索，默认经 AnySearch）/
...
> **缺模型 / 缺工具的状态码口径**：503 = 前置条件不满足（如 `@geewiki/ai-assistant` 的
> `tools_unavailable`），502 = 上游真的失败。

本节记录"AI 原生知识库"的**检索地基 + 工具与会话层**：检索与生成彻底解耦，各插件都在 `packages/server/src/index.ts` 的 `defaultRegistry()` 静态登记，且**都不进任何冲突组**。**缺模型不等于降级出假答案**：默认出厂状态下没有可用 provider，AI 功能明确 **503 不可用**（见 §9.7）。

### 9.1 检索与 AI 插件清单

| 插件 | 包路径 | `provides` | `requires`（服务 token） | 默认部署 | 对外端点 |
| --- | --- | --- | --- | --- | --- |
| `@geewiki/search` | `packages/plugin-search/` | `search-service` | `['database-provider', 'http-service', 'policy-service']`（**`policy-service` 是 P2 读路径依赖**：返回任何命中之前必须先问策略层「这个主体能看哪些条目」，`packages/plugin-search/src/index.ts:66`） | **启用**（`config/plugins.base.json` 第 9 条） | `GET /api/search?q=&limit=&mode=` |
...
| `@geewiki/openai` | `packages/plugin-openai/` | **`provides` 为空**（不声明 token） | `['llm-service']` | **启用** | 无（只向 `llm-service` 注册一条 OpenAI 兼容路由；不声明 `conflictGroup`——多个适配器是并列服务商，见 §3） |
| `@geewiki/ai-writing` | `packages/plugin-ai-writing/` | **`provides` 为空**（纯贡献者，不 provide 任何服务） | `['ai-tool-service']` | **启用** | 无端点：只向 ai-tool-service 贡献 `editor.*` 客户端工具描述符（`packages/plugin-ai-writing/src/index.ts:26-29`；**没有 client、不贡献任何插槽**，见 §9.6） |
| `@geewiki/ai-tools` | `packages/plugin-ai-tools/` | `ai-tool-service` | **无**（`requires: undefined`——它是依赖图的根之一，必须能被最先激活的那批插件接纳） | **启用** | 无端点：只暴露一张**工具注册表**（`contribute` / `list(principal)` / `ownerOf` / `release` / `diagnostics`） |
| `@geewiki/ai-kb` | `packages/plugin-ai-kb/` | **`provides` 为空**（纯贡献者） | `['ai-tool-service', 'wiki-service', 'search-service']` | **启用** | 无端点：向工具总线注册 `list_pages` / `search_kb` / `read_page`（三条都是 `side: 'server'` 的**只读**工具，无 `mutating` 标记） |
| `@geewiki/ai-journal` | `packages/plugin-ai-journal/` | `ai-journal-service` | `['http-service', 'database-provider']`（**刻意不依赖 llm / ai-tools**：它是工具的下游，反过来依赖会成环） | **启用** | `POST/GET /api/ai/journal`、`POST /api/ai/journal/undo`、`POST /api/ai/journal/undo/ack`（四个端点一律要求登录主体） |
| `@geewiki/ai-pages` | `packages/plugin-ai-pages/` | **`provides` 为空**（纯贡献者） | `['ai-tool-service', 'wiki-service', 'policy-service', 'ai-journal-service']` | **启用** | 无端点：向工具总线注册 `page.update`（本仓**第一条 `mutating: true`** 的工具，逆操作由 journal 的 `page` 域撤销执行体提供） |
| `@geewiki/builtin-docs` | `packages/plugin-builtin-docs/` | `builtin-docs-service` | `['database-provider', 'wiki-service']`（**刻意不依赖 http-service**：本包**零端点**——读走 wiki 的 `GET /api/pages/:slug`，管理走 manager 的启停与配置面） | **启用** | 无端点：`ctx.provide('builtin-docs-service')`（`isManagedPage` / `isHidden`）供 `@geewiki/authz` **懒取**；迁移 `builtin_docs_state` 是方言中立的键值表（整条链路只登记 sqlite 回退目录） |
| `@geewiki/ai-web-search` | `packages/plugin-ai-web-search/` | **`provides` 为空**（纯贡献者） | `['ai-tool-service']`（**刻意不依赖 wiki / search / policy**：它查的是**站外**，与知识库无关，也不做权限判定——能不能用由工具总线按主体过滤） | **启用** | 无端点：向工具总线注册 `web_search`（`side: 'server'` 的**只读**工具，无 `mutating` 标记）。出站只有一件事：`POST {baseUrl}/v1/search`（默认 AnySearch，`/v1/extract` 等其它端点**未接线**）。**未配密钥时走匿名额度**（按来源 IP 计），故"能装就能用"；结果声明 `grounding: 'web'`——`AiToolGrounding` 的第二个取值由本包连同界面标注一起定义 |

> **两条与本节清单相关的实现事实**：① `@geewiki/ai-journal` 的迁移目录由 `defaultRegistry()` 的 `migrationsDirs` 声明，**按方言分家**——`packages/plugin-ai-journal/migrations/` 是 SQLite 版，`migrations-postgres/` 是 PG 版（清单声明 `migrations: { default, postgres }`），调用点 `await asAsync(db).migrate(journalMigrationsDirFor(db.dialect))`。② `WikiService.save(slug, input)` **不带主体**——四个读方法都要求主体，唯独写方法没有，授权发生在 HTTP 处理器里。故 `@geewiki/ai-pages` 必须自己向 `policy-service` 要 `canEdit` 判据（那正是判据的唯一出口），并 `requires` 它以便"拿不到判据就激活失败"。见 [design/ai-plugin-architecture.md](design/ai-plugin-architecture.md) §8.8。

本节表内的 AI 插件（`search` / `llm` / `openai` / `ai-tools` / `ai-kb` / `ai-journal` / `ai-pages` / `ai-summary` / `ai-web-search` / `ai-assistant` / `ai-writing` / `ai-nav` / `ai-admin`）都声明 `runtime.supportsHotReload: true`、`requiresCachePurge: false`、`drainTimeout: 5`，均无进程内状态（索引在库里、路由注册表在内存但可安全重建），故可安全热插拔。**但"热插拔"与"能否在管理台点停用"不是一回事**：它们都写在基础层清单里，而基础层插件现在可以**临时停用**（只登记在内存、不落盘，重启即恢复，见 §5.3）；反过来**启用**一个未激活的冷插件仍会被 `hot_reload_not_supported` 拒绝。`@geewiki/search` 另带 `migrations: './migrations'`（`SEARCH_MIGRATIONS_DIR`），由**管理器的迁移控制器在激活前执行**（见 5.5），不依赖 `db-sqlite` 自己的迁移。
...
3. **新增插件一律"声明 token + 显式 provide + 同名"**：服务提供方（`search` / `llm` / `ai-tools` / `ai-journal` / `builtin-docs` 等）的 `provides` token 与服务名**严格同名**，并在源码注释里写明"两者名字必须一致，否则消费方 `ctx.get` 拿到 `undefined`"。纯贡献者（`ai-kb` / `ai-pages` / `ai-summary` / `ai-web-search` / `ai-writing` / `ai-nav` / `ai-admin` / `openai`）`provides` 为空，不提供任何服务。
...
**本包至今不含任何厂商 adapter**——adapter 在兄弟包 `@geewiki/openai`（只向 `llm-service` 注册一条 OpenAI 兼容路由，`provides` 为空，默认启用）。本包保留的是与厂商无关的三件事：route→provider 注册表与终止保证、**降级词汇的唯一真源**（`degrade.ts` 的 `Degraded`/`DegradedReason`/`CODE_TO_REASON`/`makeDegraded` + `redact.ts`）、以及**可用性投影**（`availability.ts` 的 `safeAvailable`/`listRouteInfos`/`hasAvailableModel`/`noModelDegraded`）。**"注册表里有没有路由"与"路由可用不可用"是两件事**：注册表里**有两条路由**——恒不可用的兜底占位 `null` 与默认启用的 `openai`。但 **`GET /api/llm/providers` 刻意把兜底路由过滤掉**（`packages/plugin-llm/src/index.ts:295` 起：`.filter((d) => d.vendor !== 'none')`，注释原文"内置兜底路由不是'可选服务商'，它永远不可用，列进下拉只会让人误选"），所以该端点当前**只返回 `openai` 一条**，且 **`available:false`**（`openai` 那条的 `apiKeyEnv` 是空串 ⇒ 缺凭据）。于是"没有可用模型"这个结论成立，但**成因不是"表里没有 provider"**——把成因写错会让人去查注册表而不是去查凭据。兜底占位 `NULL_PROVIDER` 在 `packages/plugin-llm/src/service.ts:102`（`route: 'null'`、`label: '未配置（降级占位）'`、`available: () => false`，被点名时只产一个终止 chunk `error{MISSING_CREDENTIAL}`）——它的职责是**让"没配置"成为一条可被枚举、可被如实报告的路由**，而不是让调用静默返回空。
...
**`@geewiki/ai-writing` 当前形态**：**没有任何 HTTP 端点、没有 `client`、不贡献任何插槽、也不 provide 服务**
（`provides: undefined`），只向 `ai-tool-service` 贡献四条 `editor.*` **客户端**工具描述符
（`packages/plugin-ai-writing/src/index.ts:26-29`：读草稿 / 读选区 / 插入 / 替换选区，`side: 'client'`，
执行体由宿主 `packages/web/src/lib/clientTools.ts` 登记）。决策 18 删掉了原先的四个按钮、
`POST /api/ai/assist` 端点、`editor-toolbar` 插槽贡献与整份前端产物——"插件自带一套写作 UI"与
"AI 只有一个入口（`app-dock` 输入条）"矛盾。

**两道仍然成立的后端红线**（现在由消费方继承）：① 进模型的正文**只来自请求体或工具入参**；
② `policy-service` 缺失 ⇒ **失败关闭**（不是"跳过检查"），且**权限判定先于降级判定**——
否则"降级"会成了一条无需权限的探测通道。

**前端归属**：宿主 `packages/web/` 里**没有** AI 面板与写作工具条——`components/AskPanel.tsx`、
`components/ai/AssistToolbar.tsx`、`lib/aiStreamPlan.ts`、`lib/assistPlan.ts`、`api.ts` 的 AI 方法与
`styles.css` 的 `.ask-*` 都已删除。界面住在各插件的 `ui/`，经**插槽**插入宿主：当前是
`@geewiki/ai-assistant` → `app-dock`（常驻输入条 + 向上展开的对话区）与
`@geewiki/ai-summary` → `article-summary`（折叠摘要卡）。**宿主不替插件说话，但也不是"零 AI 文案"**——
准确口径是"**不写插件名 / 模型名、不猜不可用的原因**"：入口按钮的文案是**功能名**（如「AI 问答」），
由宿主渲染并刻意不写插件名（`packages/web/src/pages/WikiPage.tsx:870-874` 的注释就是这条裁决）；
没有贡献者时那句「问答功能未启用」也**不点名插件**。

**守卫归属要分三方，别把插件侧守卫当成宿主侧守卫**：`packages/plugin-ai-assistant/test/uiDock*.test.ts`
与 `packages/plugin-ai-summary/test/uiCardOrder.test.ts` 钉的是**插件自己的**产物（字符串、状态分支、
镜像词汇），它们**不读 `packages/web/src`**；钉"宿主侧不再残留 AI 界面"的是
`packages/web/test/pluginUi.test.ts`（清单声明的产物名与插槽、构建链 `FIXTURE_OUT` 咬合、
**样式归属**——宿主 `styles.css` 里若残留 `.ask-*` 规则即变红、产物只能从 react 取运行时、
`ui/` 目录不得 import 服务端）；"中性占位不点名插件"则由**浏览器侧断言**钉
（`scripts/acceptance/ai-split-e2e/cdp-ui.mjs`）。
...
**没有 `dir`**（`packages/manager/src/deps.ts` 只给外部插件挂资产根），"从插件目录直接发资产"那条资产根对
内置插件根本不可用。**宿主 SDK 提供 `renderMarkdown`**（`HOST_SDK_VERSION` 现为 **`0.9.0`**）——
全仓只留一份 marked/DOMPurify；插件必须用 `typeof host.renderMarkdown === 'function'` **特性探测**，
缺席退回纯文本，**不要按 version 字符串比大小**。

**两条踩过的教训**（都已写进代码注释，都是"看起来像产品缺陷"的那类）：
① 入口可见性只能按入口表的**声明 + 仲裁**判（`pluginUiDeclaredFor`，`packages/web/src/lib/pluginUi.ts:190`；
`slots` 字段是后端裁决后的**生效集**）。按"已注册组件"判在懒加载插槽下会**自锁**——按钮永远不出现，
除非它已经出现过。② 懒加载集合 `ON_DEMAND_SLOTS` 现为 **5 项**
（`['editor', 'editor-toolbar', 'app-dock', 'article-summary', 'account-identities']`，
`packages/web/src/lib/pluginUiPlan.ts:172`）；进编辑视图必须**同时**拉起 `editor` 与 `editor-toolbar`，
漏后者得到的是**一片静默空白**：管理台里插件 active、入口表条目齐、零 console error，
与"该插件没提供界面"长得一模一样。

**"停用插件不需要重建前端"已实测**：拦截路径是**临时停用**（基础层插件可临时停用，见 §5.3，重启即恢复）；
验收比对了**产物指纹**——`packages/web/dist/index.html` 与插件 bundle 的 SHA-256 **逐字节不变**，
同时入口消失、界面出现宿主中性占位（`scripts/acceptance/ai-split-e2e/cdp-ui.mjs`，19/19）。
...
| **密钥** | 边界（**已收窄**） | 主路 `apiKey` 是**写一次、不可回读**的 secret 字段 ⇒ 落 `config/secrets.json`（0600、gitignored），`GET /api/plugins/:name/config` 该字段恒回空串 + `secrets.apiKey: true`，故**不再构成回显面**；备选路 `apiKeyEnv` 只是环境变量名，也不入库。**仍成立的边界**：`GET /api/plugins/:name/config` 对**普通配置字段**（`baseUrl` / `model` / `contextWindow` …）依然**明文返回**（见 5.7）——写一次语义只保护被标成 secret 的字段 |
...
| **镜像守卫的归属** | 已改判 | 旧做法是让宿主 web 替插件守镜像（`packages/web/test/degradedReason.test.ts`），而界面迁走后宿主**不再消费任何 AI 字段**，那个守卫就变成了"没人用的东西守着一份没人用的词汇"。**已删除**，改由消费方自守：`packages/plugin-llm/test/degrade.test.ts`（`CODE_TO_REASON` 对 `LlmErrorCode` 全量覆盖 + `search_unavailable` 钉在 union 外 + `listRouteInfos` 脱敏）与两插件界面侧守卫。留下的判据是**谁消费，谁守镜像** |
