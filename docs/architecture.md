...
6. **无版本协商与完整性校验**：插件 UI 目前没有签名与版本校验（`packages/web/src/lib/pluginUi.ts:41` 明确登记为未做的边界）。宿主 SDK 有 `HOST_SDK_VERSION`（现 **0.11.0**）与 `propsVersion`，但**没有强制的版本协商**——插件应特性探测而不是比版本号。
