...
**全部阶段已实现并合入 `main`。`main` = `5536bfa`（`merge(ops): P4 审计与运维闭环 —— 越权告警、会话管理与过期回收`）。**
**已合入：P0 / P1 / 文档 v7 / P1.5 / P2（含 M5 前三项）/ P3a / P3b / P3c / P3d / P4** —— 即设计文档 §8.1 阶段表里的**全部阶段**。
**设计文档已回写为 v8**：本轮把实现反馈的更正写进了 `access-control.md`（含一处**安全相关的方向性错误**订正、一条**新增的跨阶段不变量 §4.6**），见其 **§13.6**。

### ★ 合并后的验证基线（`main` = `5536bfa`）

`pnpm typecheck` **exit 0（17 个包）**；`pnpm test` **870 例 / 870 通过 / 0 失败**；`pnpm build` **exit 0**；
六条 e2e **共 282 项断言零失败**：`e2e-p1` **38**、`e2e-p15` **44**、`e2e-p2-org` **44**、`e2e-p2` **34**、`e2e-p3a`（SQLite）**85 / 0 / 0**、`e2e-p4` **37**。

> **复跑说明（不要混淆）**：`typecheck` / `test` / `build` 三条**已由文档 v8 那一轮实际复跑**（870/0 就是复跑结果）；
> **六条 e2e 未复跑**，断言数来自编排者的记录（命令见下方「验证基线（复现用）」）。
>
> **★ `main` 在我方复跑期间又前进了一个提交**：`5536bfa` → **`49f8f8a`**
> （`docs(plugin-search): 订正空授权集合 SQL 处理的陈旧注释`，2026-09-12 06:46:18，**只改注释、只动 `packages/plugin-search/src/index.ts`**）。
> 该提交**与本次文档回写无交集**，也不改变测试例数；**若你要复核基线，请以你自己当时的 `HEAD` 为准，并把提交号写下来。**
...
| 4 | 未登录进站应有主页面 | ✅ 服务端门户 `/portal` 完成；✅ 前端导航能力驱动已完成（M5，**前三项**；红链三步态仍未做，见下） |
