...
- **节点目录**：`packages/core/src/extensions.ts` 的 `HOST_NODE_CATALOG`。`kind` 四类：`slot`（既有 7 个，
  ...
  `ui`（15 个 `ui-*` 原语，在**定义处**接线：11 个叶子元素三模式全开，`ui-dialog-content` / `ui-dropdown-menu-content` / `ui-confirm-dialog` / `ui-tooltip` 这 4 个 **portal 类**只开 `extend` + `replace`——`wrap` 的包装元素装不下 portal 内容，属静默失效，故不提供）。
