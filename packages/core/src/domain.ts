...
/* ==================== 插件页面路由：id 语法与宿主保留 id ==================== */

/*
 * ★ 这两项原先定义在 `core/src/index.ts`，而 `packages/web` 需要它们，于是在
 * `web/src/lib/pluginUiPlan.ts` 里抄了第二份 —— 一份**有守卫的镜像**（逐元素相等）。
 *
 * 有守卫不等于没问题：守卫只在**漂移之后**才红，而"内容相等"在有人刚抄完一份时是
 * **通过**的。本仓库对插槽名做过同一次治理（优化点 1），结论相同 ——
 * 把真源搬进**浏览器安全模块**，让 web 侧**转出**它，再以**引用同一性**为判据：
 * **副本无法伪装成同一个对象**。
 *
 * 之所以放这里而不是 `index.ts`：本模块零 `node:*` / 零 cordis，前端可以 `import type`
 * 与直接 `import`；`index.ts` 顶层有 `node:fs`，进不了浏览器 bundle。
 *
 * 注意这条**就是安全边界**（不是洁癖）：后端拒绝插件声明保留 id，前端（运行期注册）
