 ...
 *
 * ⚠️ 本路径**保留原生 confirm**：页面里的危险操作（删除页面、吊销会话等）已统一换成
 * `ui/ConfirmDialog`，但那是 React 状态驱动的**异步**确认，阻塞不住"离开还是留下"这个
 * 同步判定（`hashchange` / `beforeunload` 回调里没有可等待的渲染时机）——原因见下。
 * 也就是说：这里不是"还没迁移"，而是**不能**迁移。
 *
