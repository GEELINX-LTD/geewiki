 ...
 * 现在宿主只保留真正属于核心的「本地口令」，外部身份的**数据与界面一起**搬到这里。
 *
 * ## 为什么数据由插件自己取
 ...
 * 3. **服务端才是权威判据**：没有口令且只剩一个身份时服务端返回 409 `last_credential`。
      ...
      <section className="gw-oidc-card" aria-label="外部身份">
        ...
        {view.kind === 'ready' && view.identities.length === 0 && (
          <div className="gw-oidc-state">
            ...
            {/*
              空态文案按"还有没有别的登录方式"分叉：有口令的人不需要被催着设口令，
              没有口令的人则是**只剩外部身份**这一条路 —— 那句话才是他真正需要的下一步。
            */}
